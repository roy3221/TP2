...
public class UserDetailsActivity extends BaseActivity implements HasComponent<UserComponent> {
  ...
  public static Intent getCallingIntent(Context context, int id) {
    ...
    callingIntent.putExtra(INTENT_EXTRA_PARAM_USER_ID, id);
  ...
  @Override protected void onSaveInstanceState(Bundle outState) {
    if (outState != null) {
      outState.putInt(INSTANCE_STATE_PARAM_USER_ID, this.id);
    }
  ...
  private void initializeActivity(Bundle savedInstanceState) {
    if (savedInstanceState == null) {
      this.id = getIntent().getIntExtra(INTENT_EXTRA_PARAM_USER_ID, -1);
      addFragment(R.id.fragmentContainer, new UserDetailsFragment());
    } else {
      this.id = savedInstanceState.getInt(INSTANCE_STATE_PARAM_USER_ID);
    }
  }
  ...
  private void initializeInjector() {
    this.userComponent = DaggerUserComponent.builder()
        ...
        .userModule(new UserModule(this.id))
        .build();
  }
