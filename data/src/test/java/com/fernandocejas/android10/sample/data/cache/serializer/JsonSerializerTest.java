...
public class JsonSerializerTest extends ApplicationTestCase {
  ...
  public void testSerializeHappyCase() {
    ProductEntity userEntityOne = jsonSerializer.deserialize(JSON_RESPONSE);
    String jsonString = jsonSerializer.serialize(userEntityOne);
    ProductEntity userEntityTwo = jsonSerializer.deserialize(jsonString);

    assertThat(userEntityOne.getId(), is(userEntityTwo.getId()));
    assertThat(userEntityOne.getImage_height(), is(equalTo(userEntityTwo.getImage_height())));
    assertThat(userEntityOne.getImage_url(), is(userEntityTwo.getImage_url()));
  }

  @Test
  public void testDesearializeHappyCase() {
    ProductEntity userEntity = jsonSerializer.deserialize(JSON_RESPONSE);
